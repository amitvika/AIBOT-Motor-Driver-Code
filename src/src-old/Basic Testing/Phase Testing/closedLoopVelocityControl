...
const float TARGET_RPS      = 100.0f;  // 1 rev/sec
...
void setup() {
  Serial.begin(115200);
  delay(200);
  Serial.println(F("\nSimpleFOC + AS5047P (custom SPI pins)"));
  ...
  // Starter tuning (adjust later)
  motor.PID_velocity.P  = 0.2f;
  motor.PID_velocity.I  = 5.0f;
  motor.PID_velocity.D  = 0.0f;
  motor.LPF_velocity.Tf = 0.02f;

  // Enable monitoring (prints target, vel, angle)
  ...
  Serial.println(F("FOC ready – running closed-loop velocity @ 1 rps"));
}

void loop() {
  motor.loopFOC();
  motor.move(2.0f * PI * TARGET_RPS);
